---
title: "Microarray Data Rspo KO vs. WT in  colon - GSEA analysis on DGE results - Plots"
...
We here use GSEA data on DGE results from the 44k arrays of Rspo_KO vs. WT. For analysis, a preranked analysis using the R package fGSEA is used that should give similar results to preranked analysis in standard GSEA. 

```{r}
load(file.path(result_folder, "DGE_results_Rspo_KO.Rdata"))
```
...
The plots below use differential gene expression data from 2019-05-27 and GSEA results.
...
## Comparison Axin2+ vs Axin2-

### Human gene sets

```{r}
selected_genesets = c("GO_MITOTIC_RECOMBINATION")
```


```{r}
sel_comparison = "Rspo.KO.distal_vs_WT.distal"
```
...
for (g in selected_genesets) {
  ...
  pdf(file=file.path(result_folder, paste0("Rspo_KO_vs_WT_",g, ".pdf")), width=8, height = 6)
  print(p + ggtitle(g, paste("FDR=",prettyNum(rr$FDR_global, digits=2),", p=",prettyNum(rr$pval, digits=2), ", ES=",prettyNum(rr$ES, digits=2), ", NES=",prettyNum(rr$NES, digits=2) ) ) )
  dev.off()
}
```
...
```{r}
selected_genesets = c("Lgr5_SC_Signature_Munoz")
```
