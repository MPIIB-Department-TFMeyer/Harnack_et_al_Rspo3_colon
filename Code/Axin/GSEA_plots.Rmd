---
title: "Microarray Data Axin2+/- in  colon - GSEA analysis on DGE results - Plots"
...
We here use GSEA data on DGE results from the 44k arrays of Axin2+ vs. Axin2- cells. For analysis, a preranked analysis using the R package fGSEA is used that should give similar results to preranked analysis in standard GSEA. 

```{r}
load(file.path(result_folder, "DGE_results_Axin.Rdata"))
```
...
```{r}
sel_comparison = "Axin_p_vs_Axin_m"
```
...
for (g in selected_genesets) {
  ...
  pdf(file=file.path(result_folder, paste0("Axin2_plus_vs_minus_",g, ".pdf")), width=8, height = 6)
  print(p + ggtitle(g, paste("FDR=",prettyNum(rr$FDR_global, digits=2),", p=",prettyNum(rr$pval, digits=2), ", ES=",prettyNum(rr$ES, digits=2), ", NES=",prettyNum(rr$NES, digits=2) ) ) )
  dev.off()
}
```
